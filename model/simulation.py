...
class Simulation(Shared):
    ...
    def generateHistory(self, start_week: int, end_week: int, smoothing_filter: SmoothingFilter=None):
        ...
        stock_ini["cdc"] = {p: 1000 for p in self.products}

        cdemand_ini = {a: {} for a in self.itAffiliates()}
        for a in self.itAffiliates():
            ...
            for p in self.itAffProducts(a):
                ...
                cdemand_ini[a][p] = utils.genRandCQFromUCM(self.risk_manager.d_model[a][p], cdemand_ini_, 0)
                cdemand_ini[a][p] += (self.horizon-self.real_horizon)*[cdemand_ini[a][p][self.real_horizon-1]]
                
        r0 = sum([self.getAffPvRange(a) for a in self.itAffiliates()])
        crecep_ini = {}
        for p in self.products:
            crecep_ini_ = utils.genRandCQ(self.horizon, r0)
            crecep_ini[p] = utils.genRandCQFromUCM(self.risk_manager.r_model[p], crecep_ini_, 0)
        ...
        for w in range(start_week, end_week + 1):
            ...
            cpr = {p: cproduct_supply[p][0] for p in self.products }
            prev_cpsupplly = {p: list(utils.accumu(prev_psupply[p], cpr[p])) for p in self.products}
            cproduct_supply = {p: list(utils.accumu(product_supply[p], cpr[p])) for p in self.products}
            ...
            if smoothing_filter:
                ...
                product_supply_out = {p: utils.diff(cproduct_supply_out[p], cpr[p]) + product_supply[p][n:] for p in self.products}
