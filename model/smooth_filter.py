...
class SmoothFilter:
    ...
    def filter(self, risk_m: RiskManager, rpm: dict, dpm: dict, s0: int, x_in: list):
        ...
        while solved | unsolvable != to_solve:
            ...
            print("-------------------------------------------------------------------------")
            if n_iter > max_iter:
                raise Exception(to_solve, solved, unsolvable)
            for t in to_solve - unsolvable:
                ...
                if c >= b:
                    if x[t] < b:
                        x[t] = min(b, x[t+1])
                        if t == n-2 and x[t] == x[n-1] and l4n_in[n-1] >= self.alpha:
                            unsolvable.add(t)

                    elif x[t] > c:
                        x[t] = max(c, x[t-1])
                        if any(t == k and x[k] == x[k-1] and l4n_in[k-1] >= self.alpha for k in [1, 2]):
                            unsolvable.add(t)
                else:
